...
var blackManWindow = function(size) {
	...
	for (var i=0; i<win.length; i++) {
		win[i] = 0.42 - 0.5 * Math.cos(2 * Math.PI * i / (win.length - 1)) + 0.08 * Math.cos(4 * Math.PI * i / (win.length - 1));
...
function Analyser() {
	...
	this.response = function(i) {
		if ((i * (this.audioContext.sampleRate / this.bufSize)) < 1) {
					return ((-1 * 0.06 * (this.audioContext.sampleRate / this.bufSize) * i) + 13);
		} else {
			return 1.0;
		}
	}
	...
	this.computeFrequency = function() {
		var frequencies = this.getData().subarray(0, 500 / (this.audioContext.sampleRate / this.bufSize));

		var frequency = (frequencies.indexOf(getMax(frequencies)) * (this.audioContext.sampleRate / this.bufSize));
		return frequency;
	}
	...
	this.displayArrow = function(value, display) {
		...
		if (currentSelection != 0) {
			for (var i=0; i<tuningPatterns.strings.length; i++) {
				difference = Math.abs(this.finalFrequency - parseFloat(tuningPatterns.strings[i][1]));
